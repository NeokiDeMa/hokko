...
public struct Offer<phantom T: key + store> has key, store {
    ...
    owner: address,
    item: ID,
    // df_infos: VecMap<Name, ID>, // key : name of dynamic field, value : id of
    // object
    price: u64,
    royalty_fee: u64,
    market_fee: u64,
...
///      This includes initializing the required offer object, configuring the
/// kiosk extension if necessary,
///      and emitting an event for the created offer.
/// @param kiosk A mutable reference to the kiosk where the offer will be
/// stored.
...
public fun offer<T: key + store>(
    ...
    // if kiosk has no extension or extension is disabled -> install or enable
    // extension
...
/// @dev Revokes an existing offer in the marketplace. This function removes the
/// offer from
///      the kiosk's storage, deletes the offer and its capability, and refunds
/// the balance to
///      the sender.
...
/// @param offer_cap The capability object associated with the offer being
/// revoked.
/// @param ctx The transaction context of the sender.
#[allow(lint(self_transfer))]
...
/// @dev Accepts an offer in the marketplace. Transfers the item to the
/// accepter's kiosk,
///      deducts and distributes fees (market and royalty), and processes the
/// item transfer based
///      on the associated transfer policy.
/// @notice After calling `accept_offer`, the client-side must fulfill the
/// custom transfer policy rules.
///         The `confirm_offer_accepted` function should be executed last to
/// finalize the offer acceptance.
///         The `OfferWrapper` is a hotpotato and should be processed in
/// `confirm_offer_accepted`.
/// @param offerer_kiosk A mutable reference to the kiosk of the offerer where
/// the offer exists.
/// @param offer_id The ID of the offer being accepted.
/// @param accepter_kiosk A mutable reference to the kiosk of the accepter where
/// the item was stored.
/// @param accepter_kiosk_cap A capability object that allows modification of
/// the accepter's kiosk.
/// @param item_id The ID of the item being transferred.
/// @param policy A mutable reference to the transfer policy applied to the
/// item.
/// @param market A mutable reference to the marketplace where the market fee is
/// paid.
/// @param ctx The transaction context of the sender.
/// @return A tuple containing an `OfferWrapper<T>` for the accepted offer and a
/// `TransferRequest<T>` for the item transfer.
public fun accept_offer<T: key + store>(
    ...
    let (item, mut request) = accepter_kiosk.purchase<T>(
        item_id,
        item_price_coin,
    );
    if (royalty_fee > 0) {
    ...
    assert!(
        kiosk_extension::can_place<Ext>(offerer_kiosk),
        ECanNotPlaceToExtension,
    );
...
/// @dev Confirms the acceptance of an offer, emits an event, destroys the offer
/// object, and refunds
///      any remaining balance to the offerer.
/// @notice The `confirm_offer_accepted` function should be executed last to
/// finalize the offer acceptance.
///         The `OfferWrapper` is a hotpotato and should be processed in
/// `confirm_offer_accepted`.
...
/// @dev Declines an existing offer in the marketplace. This function removes
/// the offer from
///      the kiosk's storage, refunds the offer's balance to the offerer, and
/// emits a decline event.
/// @param offerer_kiosk A mutable reference to the kiosk where the offer was
/// stored.
...
/// @dev Creates a new offer for an item in the marketplace. This function
/// calculates
///      the market fee and royalty fee, ensures the payment covers all required
/// fees, and
...
/// @param ctx The transaction context of the sender.
/// @return A tuple containing the created `Offer<T>` and its associated
/// `OfferCap<T>`.
fun new_offer<T: key + store>(
    ...
    assert!(
        payment.value() >= price + market_fee + royalty_fee,
        EInsufficientAmount,
    );
...
/// @dev Emits an event when a new offer is created in the marketplace. This
/// event contains
///      information about the offer, including the kiosk, offer ID, item,
...
/// @dev Emits an event when an offer is revoked in the marketplace. This event
/// contains
///      information about the revoked offer, including the kiosk, offer ID,
/// item, price, and associated fees.
...
/// @dev Emits an event when an offer is accepted in the marketplace. This event
/// contains
///      information about the accepted offer, including the kiosk, offer ID,
/// item, price, and associated fees.
...
/// @dev Emits an event when an offer is declined in the marketplace. This event
/// contains
///      information about the declined offer, including the kiosk, offer ID,
/// item, price, and associated fees.
...
// ====================== Test ======================
/// @dev Returns the offer ID from an `OfferEvent`. This function is used for
/// retrieving
///      the unique identifier of an offer from an event to test.
/// @param event A reference to the `OfferEvent` object containing the offer
/// details.
