...
/// @dev This function creates a new kiosk along with a personal kiosk
/// capability (PersonalKioskCap).
///      It sets the kiosk owner, transfers ownership of the kiosk capability to
/// the sender, and emits a KioskCreatedEvent.
/// @notice This function does not store the created kiosk information in the
/// marketplace. It simply creates a kiosk.
...
/// @dev This function lists an item in a kiosk on the marketplace with purchase
/// cap,
///      and stores price and fee information in the shared purchase cap.
/// @notice This function lists an item placed in a kiosk.
/// @param market The reference to the marketplace where the item is to be
/// listed.
...
public fun list_kiosk_item<T: key + store>(
    ...
    let marketplace_fee =
        (
            ((get_fee(market, tx_context::sender(ctx)) as u128) * (price as u128)) / 10000,
        ) as u64;
...
/// @dev This function updates an existing kiosk listing by modifying its price
/// and recalculating its fees.
/// @param market The reference to the marketplace where the item is listed.
/// @param s_item_pc A mutable reference to the `SItemWithPurchaseCap`
/// representing the listed item to be updated.
/// @param kiosk_cap A capability object that allows modification of the kiosk.
/// @param kiosk A mutable reference to the kiosk from which the item is listed.
/// @param tp The transfer policy associated with the listed item.
...
/// @dev This function delists an item from a kiosk, ensuring all required
/// permissions and checks
///      are passed before performing the delisting action. The purchase cap is
/// returned and the item is no longer
///      available for purchase through the kiosk.
/// @param s_item_pc A struct containing the item details, purchase cap, and
/// ownership information.
/// @param kiosk_cap A capability object that allows modification of the kiosk.
/// @param kiosk A mutable reference to the kiosk where the item is being
/// delisted from.
/// @param tp The transfer policy associated with the listed item.
...
/// @dev This function facilitates the purchase of an item from a seller's
/// kiosk. It ensures the buyer has
///      sufficient funds, verifies the item and kiosk details, processes the
/// marketplace and royalty fees, and
///      transfers ownership of the item to the buyer. If applicable, it applies
/// transfer policies to the item
///      and emits an event upon successful purchase.
/// @param market A mutable reference to the marketplace handling balances and
/// marketplace fees.
/// @param buyers_kc The kiosk capability used to verify the buyer's access to
/// their kiosk.
/// @param buyers_kiosk A mutable reference to the buyer's kiosk where the
/// purchased item will be placed or locked.
/// @param sellers_kiosk A mutable reference to the seller's kiosk from which
/// the item is being purchased.
/// @param tp The transfer policy associated with the item, defining rules for
/// ownership transfer.
/// @param item The ID of the item being purchased from the seller's kiosk.
/// @param item_purchase_cap A struct containing the purchase capability, item
/// details, and pricing information.
/// @param payment the buyer's payment coin used to pay for the item and
/// associated fees.
/// @param ctx The transaction context of the sender.
/// @return TransferRequest<T> The transfer request for the item. After
/// executing this function,
/// the rules associated with the transfer policy must be collected and
/// confirmed.
#[allow(lint(self_transfer))]
public fun buy<T: key + store>(
    ...
    assert!(
        payment_balance.value() >= price + royalty_fee + marketplace_fee,
        ENotEnoughFunds,
    );
    ...
    let market_fee_coin = coin::take<SUI>(
        &mut payment_balance,
        marketplace_fee,
        ctx,
    );
    ...
    let (i, mut tr) = kiosk::purchase_with_cap<T>(
        sellers_kiosk,
        purchaseCap,
        payment_coin,
    );
    ...
    if (tp.has_rule<T, r_rule::Rule>()) {
        let royalty_fee_coin = coin::take<SUI>(
            &mut payment_balance,
            royalty_fee,
            ctx,
        );
...
/// @dev This function finalizes the purchase process by confirming the transfer
/// request for an item.
///      It ensures that all rules and policies associated with the item's
/// transfer policy have been fulfilled
///      before completing the transfer.
/// @notice This function must be called after the `buy` function to validate
/// the transfer and complete the transaction.
/// @param tr The transfer request generated during the purchase process,
/// containing details of the item and transfer rules to be confirmed.
/// @param tp A reference to the transfer policy associated with the item, which
/// defines the rules and conditions for the transfer.
public fun confirm_purchase<T: key + store>(
    tr: TransferRequest<T>,
    tp: &TransferPolicy<T>,
) {
...
/// @param self A mutable reference to the `MarketPlace` object where the fee is
/// being configured.
/// @param admin A reference to the `AdminCap` role capability for verifying
/// administrative access.
/// @param roles A reference to the roles object that stores the administrative
/// permissions for the marketplace.
/// @param recipient A vector of addresses representing the recipients to which
/// the fees will be assigned.
/// @param fee The fee rate (in basis points, where 1% = 100 basis points) to be
/// applied to the recipient's transactions.
public fun set_personal_fee(
    ...
    assert!(
        has_cap_access<MARKETPLACE, AdminCap>(roles, admin),
        ENotAuthorized,
    );
...
/// @dev Updates the base fee for all marketplace transactions. Only authorized
/// administrators can call this function.
/// @param self A mutable reference to the `MarketPlace` object where the base
/// fee will be updated.
/// @param admin A reference to the `AdminCap` role capability for verifying
/// administrative access.
/// @param roles A reference to the roles object that stores the administrative
/// permissions for the marketplace.
/// @param fee The new base fee rate (in basis points, where 1% = 100 basis
...
/// @dev Grants administrative privileges to the specified recipient. Only the
/// marketplace owner can add new administrators.
/// @param owner A reference to the `OwnerCap` of the marketplace, used to
/// verify ownership and authority.
/// @param roles A mutable reference to the roles object where the recipient
/// will be granted administrative privileges.
/// @param recipient The address of the recipient who will be assigned the
/// `AdminCap`.
...
/// @dev Revokes administrative privileges from a specified administrator. Only
/// the marketplace owner can perform this action.
/// @param _ A reference to the `OwnerCap` of the marketplace, used to verify
/// ownership and authority.
/// @param roles A mutable reference to the roles object from which the target
/// administrator's privileges will be removed.
/// @param target The ID of the administrator whose privileges are to be
/// revoked.
/// @param ctx Sender's tx context.
...
/// @dev Allows an administrator to withdraw a specified amount of profit from
/// the marketplace's balance.
/// @notice If no amount is specified, the entire balance is withdrawn.
/// @param admin A reference to the `AdminCap` role capability to verify
/// administrative access.
/// @param roles A reference to the roles of the marketplace, used to check
/// authorization.
/// @param self A mutable reference to the `MarketPlace` object from which the
/// profit is being withdrawn.
/// @param amount An optional `u64` value specifying the amount to withdraw. If
/// not provided, the full balance is withdrawn.
/// @param recipient The address of the recipient to whom the withdrawn funds
/// will be transferred.
/// @param ctx Sender's tx context.
...
/// @param self A mutable reference to the `MarketPlace` object where the
/// balance will be updated.
/// @param payment A `Coin<SUI>` representing the SUI coin object to be added to
/// the marketplace's balance.
...
/// @dev Retrieves the minimum price of an item associated with its purchase
/// capability.
/// @param self A reference to the `SItemWithPurchaseCap` object containing the
/// item's details.
...
/// @dev Calculates the total fee (marketplace fee + royalty fee) for an item
/// associated with its purchase capability.
/// @param self A reference to the `SItemWithPurchaseCap` object containing the
/// item's details.
...
/// @dev Retrieves the owner address of an item associated with its purchase
/// capability.
/// @param self A reference to the `SItemWithPurchaseCap` object containing the
/// item's details.
...
/// @dev Retrieves the ID of the kiosk associated with an item's purchase
/// capability.
/// @param self A reference to the `SItemWithPurchaseCap` object containing the
/// item's details.
...
/// @dev Retrieves the current balance of the marketplace. Only accessible to
/// authorized administrators.
/// @param self A reference to the `MarketPlace` object containing the balance
/// details.
/// @param roles A reference to the roles object verifying administrative
/// access.
/// @param admin A reference to the `AdminCap` role capability for access
/// validation.
...
public fun get_purchase_cap_id<T: key + store>(
    self: &SItemWithPurchaseCap<T>,
): ID {
    object::id(&self.purchaseCap)
}

/// @dev Retrieves the fee applicable to a specific owner in the marketplace. If
/// a personal fee exists for the owner, it is returned; otherwise, the base fee
/// is used.
/// @param self A reference to the `MarketPlace` object containing the fee
/// configuration.
...
public fun get_fee(self: &MarketPlace, owner: address): u16 {
    let personal_fee_exists = map::contains<address, u16>(
        &self.personalFee,
        &owner,
    );
...
/// @notice Use this function only to store important information related to
/// trading
/// @param market A mutable reference to the `MarketPlace` object where the
...
public(package) fun add_to_marketplace<
...
/// @dev Removes a key-value pair from the marketplace's dynamic object fields
...
public(package) fun remove_from_marketplace<
    Name: copy + drop + store,
    Value: key + store,
>(
    market: &mut MarketPlace,
...
/// @param shared_purchaseCap The shared purchase capability ID associated with
/// the item.
/// @param item The ID of the listed item.
/// @param price The listing price of the item (in mist units).
/// @param marketplace_fee The fee charged by the marketplace for listing the
/// item.
...
/// @dev Emits an event when the details of a listed item are updated in the
/// marketplace.
/// @param kiosk The ID of the kiosk where the item is listed.
