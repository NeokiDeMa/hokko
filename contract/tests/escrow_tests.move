...
use marketplace::{
    ...
    marketplace::{Self, MarketPlace, MARKETPLACE, AdminCap}
};
use sui::{
    coin,
    event,
...
fun test_escrow() {
    ...
    let (
        ...
        s_roles,
        admin_cap,
    ) = prepare_marketplace(&mut scenario);
    ...
    let (mut policy, policy_cap) = transfer_policy::new_for_testing<
        DummyItem,
    >(scenario.ctx());
    ...
    let (mut accepter_kiosk, accepter_personal_kiosk_cap) = create_kiosk(
        &mut scenario,
    );

    scenario.next_tx(OFFER_CREATOR);
    let (mut offerer_kiosk, offerer_personal_kiosk_cap) = create_kiosk(
        &mut scenario,
    );
    let price: u64 = 1000;
...
fun prepare_marketplace(
    ...
    let owner_cap = scenario.take_from_address<OwnerCap<MARKETPLACE>>(
        MARKET_OWNER,
    );
...
fun create_kiosk(scenario: &mut Scenario): (Kiosk, PersonalKioskCap) {
    ...
    let personal_kiosk_cap = scenario.take_from_address<
        PersonalKioskCap,
    >(scenario.sender());
