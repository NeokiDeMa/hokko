...
fun test_list_item() {
    ...
    let (mut market, owner_cap, s_roles, dummy_item, admin_cap) = init_setup(
        &mut scen,
    );

    let (mut kiosk, personal_kiosk_cap) = create_kiosk(&mut scen, OWNER);
    ...
    let shared_item_purchase_cap = scen::take_shared<
        SItemWithPurchaseCap<DummyItem>,
    >(&scen);
...
fun test_purchase_item() {
    ...
    let mut scen = scen::begin(OWNER);
    let mut scenTwo = scen::begin(ALICE);

    let (mut alice_kiosk, alice_personal_kiosk_cap) = create_kiosk(
        &mut scenTwo,
        ALICE,
    );
    ...
    let (mut tp, tpc) = transfer_policy::new_for_testing<DummyItem>(
        ctx(&mut scen),
    );
    scen::next_tx(&mut scen, OWNER);
    ...
    let sui_coin_amount = mint_for_testing<SUI>(2300, ctx(&mut scenTwo));
    // let mut sui_coin_fee_amount = mint_for_testing<SUI>(300, ctx(&mut
    // scenTwo));
    // scen::next_tx(&mut scenTwo, ALICE);
    scen::next_tx(&mut scenTwo, ALICE);
    let (tr) = marketplace::buy<DummyItem>(
        &mut market,
    ...
    let (confirm_item, confirm_paid, _) = transfer_policy::confirm_request<
        DummyItem,
    >(&tp, tr);
    scen::next_tx(&mut scen, ALICE);
    let withdraw_amount = option::some(10);
    let before_balance = marketplace::get_balance(
        &market,
        &s_roles,
        &admin_cap,
    );
    ...
    let market_balance = marketplace::get_balance(
        &market,
        &s_roles,
        &admin_cap,
    );
...
fun test_update_listing() {
    ...
    let (market, owner_cap, s_roles, dummy_item, admin_cap) = init_setup(
        &mut scen,
    );
    let (mut owner_kiosk, owner_personal_kiosk_cap) = create_kiosk(
        &mut scen,
        OWNER,
    );
    ...
    let mut shared_item_purchase_cap = scen::take_shared<
        SItemWithPurchaseCap<DummyItem>,
    >(
        &scen,
    );
    ...
    marketplace::update_listing<DummyItem>(
        &market,
...
fun after_init(
    scen: &mut Scenario,
): (MarketPlace, OwnerCap<MARKETPLACE>, SRoles<MARKETPLACE>) {
...
fun create_kiosk(
    scen: &mut Scenario,
    recipient: address,
): (Kiosk, PersonalKioskCap) {
    marketplace::create_kiosk(ctx(scen));
    scen::next_tx(scen, recipient);

    let kiosk = scen::take_shared<Kiosk>(scen);
    let personal_kiosk_cap = scen::take_from_address<PersonalKioskCap>(
        scen,
        recipient,
    );
