...
fun test_purchase_simple_item() {
    ...
    let (mut market, owner_cap, mut s_roles, dummy_item) = init_setup(
        &mut scen,
    );
    scen::next_tx(&mut scen, OWNER);
    let dummy_id = object::id(&dummy_item);
    ...
    let coin_payment = mint_for_testing<SUI>(
    ...
    let marketplace_profit = marketplace::get_balance(
        &market,
        &s_roles,
        &new_admin,
    );
